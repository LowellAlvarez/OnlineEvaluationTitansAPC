...
Class Action {
	...
	function login(){
		extract($_POST);
		$type = array("","users","faculty_list","director_list");
		$type2 = array("","admin","faculty","director");
	...
	function login2(){
		extract($_POST);
			$qry = $this->db->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM director_list where director_code = '".$director_code."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
	...
	function save_director(){
		...
		$check = $this->db->query("SELECT * FROM director_list where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO director_list set $data");
		}else{
			$save = $this->db->query("UPDATE director_list set $data where id = $id");
		}
	...
	function delete_director(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM director_list where id = ".$id);
		if($delete)
			return 1;
	}
	function save_task(){
	...
	function save_evaluation(){
		extract($_POST);
		$eval_comments =  $this->db->real_escape_string($comments);
		$data = " director_id = {$_SESSION['login_id']} ";
		$data .= ", academic_id = $academic_id ";
		// $data .= ", subject_id = $subject_id ";
		// $data .= ", class_id = $class_id ";
				$data .= ", comments = '$eval_comments' ";
	...
	function get_report(){
		...
		// 	$rate = array();
		$get = $this->db->query("SELECT comments FROM evaluation_list where academic_id = {$_SESSION['academic']['id']} and faculty_id = {$faculty_id} ");
		$comments=array();
		while($row = $get->fetch_assoc()){
		...
		$get = $this->db->query("SELECT sum(evaluation_answers.rate) as total_rate,evaluation_answers.question_id,(SELECT count(*) from evaluation_list where academic_id={$_SESSION['academic']['id']} and faculty_id={$faculty_id}) as number_eval FROM `evaluation_list` LEFT JOIN evaluation_answers ON evaluation_list.evaluation_id=evaluation_answers.evaluation_id where academic_id={$_SESSION['academic']['id']} and faculty_id={$faculty_id} group by evaluation_answers.question_id");
		// $data[]= $row;
		...
		$total_average_ctr =0;
		while($row = $get->fetch_assoc()){
			$row['average'] = (($row['total_rate']/$row['number_eval']));
			$total_average_ctr +=1;
			$total_average += $row['average'];
			$r[$row['question_id']] = $row;
		}

		$data['total_average'] = $total_average/$total_average_ctr;
	...
	$data['data'] = $r;
	// echo "<pre>",print_r($comments),"</pre>";die();
	$data['notes'] =implode("<br>", $comments);
